...
class TokenERC721Contract extends Contract {
	...
	*/
	 ...
	 * 
	 * @param {Context} ctx 
	 * @returns All tokens owned by this (the user call from BE) owner
	 */
	async ClientAccountTokens(ctx) {
		const clientAccountID = ctx.clientIdentity.getID();
		let queryString = {};
		queryString.selector = {};
		queryString.selector.owner = clientAccountID;
	 ...
	 * 
	 * @param {Context} ctx 
	 * @param {Object} queryString a query string object
	 * @returns Key - Value object of queried document(s)
	 */
	async _getQueryResultForQueryString(ctx, queryString) {
		let resultsIterator = await ctx.stub.getQueryResult(queryString);
		let result = await this._readAllRecords(resultsIterator, false);
	...
	/**
	 * 
	 * @param {Iterator} iterator the iterable object
	 * @param {boolean} historyWanted if the call is used to get history of a key
	 * @returns An array of Key-Value objects or the 
	 */
	async _readAllRecords(iterator, historyWanted) {
		let docs = [];
